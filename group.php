...
use AnarchyService\Clsses\GoogleTranslate;
...
if (!$Group) {
    ...
    $DefaultSettings = [
        ...
        'RemDelAccountStatus' =>
            [
                'Value' => false,
                'name' => 'وضعیت حذف دیلیت اکانت (خودکار بصورت هفتگی)'
            ],
        'RemBotsStatus' =>
            [
                'Value' => false,
                'name' => 'وضعیت حذف ربات (خودکار بصورت هفتگی)'
            ],
        'RemTabChiStatus' =>
            [
                'Value' => false,
                'name' => 'وضعیت حذف تبچی (خودکار بصورت هفتگی)'
            ]
    ];
...
if ($Group->Working) {
    ...
    if (Get::$reply_to_from_id){
        ...
        if ($Group->Settings->InfoMSGStatus->Value && (preg_match('~^(ربات)? ?من$~', Get::$text) || preg_match('~^me$~i', Get::$text) || preg_match('~^info$~i', Get::$text))) {
            ...
            Send::sendMessage(Get::$chat_id, jdate('h : i'), null, false, false, Get::$message_id);
        } elseif (preg_match('~امروز ?چندمه?~', Get::$text)) {
            Send::sendMessage(Get::$chat_id, jdate('d F'), null, false, false, Get::$message_id);
        } elseif (preg_match('~امروز ?چند ?شنب~', Get::$text)) {
            Send::sendMessage(Get::$chat_id, jdate('l'), null, false, false, Get::$message_id);
